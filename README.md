...
|------|-----|-------------|
| `index.html` | `/` | **Live view** — read-only, updates in real time |
...
- **Tap** any queue tile to remove it and return it to Not in Queue
...
Identical to `stand.html` but stores data in the browser's localStorage instead of Firebase. No sync, no history, no device name prompt. Shows **SAVED** instead of LIVE. Useful for personal tracking that doesn't affect the shared queue.
